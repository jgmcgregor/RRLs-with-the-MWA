...
# Plots the noise levels vs integration time
...
#calculate rms for 64obs sample
...
phase3_correc = 0.375 / np.sqrt(20150) # 0.5 (from MWA Phase III) and 0.75 (from better observing dates)
...
ax.plot(xlist,yfit,'C0',label = 'Noise - Single freqency')
ax.plot(xlist,areafit,'C1', label = 'Noise - Single freqency, 1 square degree')
ax.plot(xlist,foldfit,'C2', label = 'Noise - Single frequency, 1 square degree, RRLs stacked')
...
#ax.plot([0.05,1000],[0.12,0.12],'C4--',label='3 sigma')
ax.plot([0.05,1000],[0.07,0.07],'C4--',label='Expected 5 sigma detection')
ax.plot([xlist[-1],xlist[-1]],[0.01,2000],'k')
ax.set_xscale('log')
ax.set_xlabel("Integration time (hours)",fontsize=15)
ax.set_xlim([0.05,1000])
ax.set_yscale('log')
ax.set_ylabel("Flux density (mJy/beam)",fontsize=15)
